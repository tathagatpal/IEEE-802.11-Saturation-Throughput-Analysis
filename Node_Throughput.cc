...
int main (int argc, char *argv[])
    ...
    cmd.AddValue ("R", "Constant Rate in Mbps", R);
 ...
 std::cout << "--------------------------"<< std::endl;
    ...
    while (count<max_it)
        ...
        //Set Backoff Windows Values
        // Create an instance of the Txop module and set the minimum and maximum CW values
        Ptr<Txop> txop = CreateObject<Txop>();
        txop->SetMinCw(cwmin);
        txop->SetMaxCw(cwmax);
        ...
        // Get the current contention window values
    int currentCwMin = txop->GetObject<Txop>()->GetMinCw ();
    int currentCwMax = txop->GetObject<Txop>()->GetMaxCw ();

    // Print or use the values as needed
    std::cout << "Current CWmin: " << currentCwMin << std::endl;
    std::cout << "Current CWmax: " << currentCwMax << std::endl;
    ...
    std::string folderName = "Plots_Simulation";
   ...
   // Create the folder if it doesn't exist
    ...
    std::string FilePath = fullDirectoryPath + "Node-Throughtput.plt";
    
    //Plot 1 Node vs Throughput
    // Instantiate the plot and set its title.
    Gnuplot plot1(FilePath);
    plot1.SetTitle("Nodes vs Thoughput");
    ...
    plot1.SetLegend("N", "Throughput (Mbps)");
    ...
    for(int i=0; i<max_it;i++)
        {
            dataset1.Add(numNodes[i], throughput[i]);
        }
